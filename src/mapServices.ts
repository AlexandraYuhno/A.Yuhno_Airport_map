...
export default class MapService {
  ...
  addAirQualityMarkers(
    airports: Airport[],
    AirportAirQuality: Record<string, AirportAirQuality>
  ) {
    if (!this.map) return;
    airports.forEach((airport) => {
      ...
      const airQualityInfo = airQuality
        ? `Показатели воздуха: 
        SO2: ${airQuality.SO2.concentration},<br> 
  ...
  addWeatherMarkers(
    airports: Airport[],
    weatherAirport: Record<string, Weather>
  ) {
    if (!this.map) return;
    airports.forEach((airport) => {
      const { latitude, longitude, icao } = airport;
      ...
      const weatherInfo = weather
        ...
        Атмосферное давление: ${weather.pressureSurfaceLevel} мм рт. ст.<br>
